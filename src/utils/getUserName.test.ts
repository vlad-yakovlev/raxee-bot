...
test('should return unescaped username', () => {
  expect(getUserName({ first_name: 'bar', id: 12345, is_bot: false, username: 'foo_foo' })).toBe('foo_foo');
});

test('should return escaped username', () => {
  expect(getUserName({ first_name: 'bar', id: 12345, is_bot: false, username: 'foo_foo' }, true)).toBe('foo\\_foo');
});
